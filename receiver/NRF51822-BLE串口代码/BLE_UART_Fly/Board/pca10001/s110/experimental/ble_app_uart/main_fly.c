...
#define DEVICE_NAME                     "euler_drone"                               /**< Name of device. Will be included in the advertising data. */
 ...
 */
static void gap_params_init(void)//设置GAP->通用接入规范
{
    uint32_t                err_code;
    ...
    gap_conn_params.slave_latency     = SLAVE_LATENCY;//从机潜伏，允许设备跳过最大连接次数，为0,能快速接收到Master发送过来的数据
    gap_conn_params.conn_sup_timeout  = CONN_SUP_TIMEOUT;//监督超时时间，超时没有收到数据 认为断开连接
    err_code = sd_ble_gap_ppcp_set(&gap_conn_params);
    APP_ERROR_CHECK(err_code);
		
		err_code = sd_ble_gap_tx_power_set(TX_POWER_LEVEL);//修改发射功率
    APP_ERROR_CHECK(err_code);
}
...
void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)//接受来自BLE的数据，然后放到UART的发送到外部
{
...
static void services_init(void)
{
    ...
    nus_init.data_handler = nus_data_handler;//uart处理函数接受来自BLE数据，然后发送到外部
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    {
        case BLE_GAP_EVT_CONNECTED://ble连接时
        ...
        case BLE_GAP_EVT_DISCONNECTED://ble断开
 ...
 * @param[in]   p_ble_evt   S110 SoftDevice event.
 */
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)//注册事件
{
    ble_conn_params_on_ble_evt(p_ble_evt);//连接管理相关event处理函数
    ble_nus_on_ble_evt(&m_nus, p_ble_evt);//串口(nus)事件
    on_ble_evt(p_ble_evt);//自定义的处理函数，包括亮灯灭灯等
}
...
static void ble_stack_init(void)
{
    ...
    uint32_t err_code = softdevice_ble_evt_handler_set(ble_evt_dispatch);//注册BLE事件
    APP_ERROR_CHECK(err_code);
}
 ...
 */
		static uint8_t data_array[30]={0,};
    static uint8_t index = 0;
		uint8_t delay_cnt=10;

void UART0_IRQHandler(void)//从UART接收到数据后 发送数据到BLE
{		
    //uint32_t err_code;
...
int main(void)
{
    ...
    buttons_init();
    uart_init();
    ble_stack_init();//初始化蓝牙协议栈并注册事件处理函数  注册函数
    gap_params_init();//设置GAP->通用接入规范  用于建立和管理连接
    services_init();//执行服务初始化  得到服务的句柄 
    advertising_init();//广播数据初始化 包括需要广播的UUID 需要广播的自定义数据以及广播类型等其他参数设置
    ...
    for (;;)
    {
			if(index!=0)
			{
				...
				if(index>20)
				{
					...
					for(cnt=20;cnt<index;cnt++)
						data_array[cnt-20]=data_array[cnt];	
					//nrf_delay_us(500);
					err_code = ble_nus_send_string(&m_nus, data_array, index-20);
					if (err_code != NRF_ERROR_INVALID_STATE)
					{
							APP_ERROR_CHECK(err_code);
					}
