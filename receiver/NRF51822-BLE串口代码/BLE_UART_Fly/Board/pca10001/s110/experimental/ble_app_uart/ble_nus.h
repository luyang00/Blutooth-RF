...
#define BLE_NUS_MAX_DATA_LEN            (GATT_MTU_SIZE_DEFAULT - 0)  /**< Maximum length of data (in bytes) that can be transmitted by the Nordic UART service module to the peer. */

#define BLE_NUS_MAX_RX_CHAR_LEN         BLE_NUS_MAX_DATA_LEN         /**< Maximum length of the RX Characteristic (in bytes). */
#define BLE_NUS_MAX_TX_CHAR_LEN         23                          /**< Maximum length of the TX Characteristic (in bytes). */
...
typedef struct ble_nus_s
{
    ...
    uint16_t                 service_handle; //服务的句柄         /**< Handle of Nordic UART Service (as provided by the S110 SoftDevice). */
    ble_gatts_char_handles_t tx_handles;     //特性的句柄        /**< Handles related to the TX characteristic. (as provided by the S110 SoftDevice)*/
